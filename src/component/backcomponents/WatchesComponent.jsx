...
const Watchescomponent=()=>{
    ...
    useEffect(()=>{
        axios.get(`http://localhost:3000/watches/${index}`)
        .then((r)=>{
    ...
    let [sectionname,setSectionname]=useState("watches")
    let addtocart=()=>{
        ...
        // navigate(`/cart/${sectionname}/${index}`)
        }
    }
    return(
        <div>
            ...
            <div id={style.pricediv}>
                    <p id={style.price}>{price}   offer!!!</p> <p id={style.oldprice}>₹8500</p></div>
                    </div>
                </div>
            </div>
        </div>
